package com.pgzxc.customsetting.widget;
...
import com.pgzxc.customsetting.R;

/**
 * 我的 ui中item
 */
public class InfoItemTextView extends FrameLayout {
    ...
    public InfoItemTextView(Context context) {
        this(context, null);
    }

    public InfoItemTextView(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    private void initView(AttributeSet attrs) {
        View view = View.inflate(mContext, R.layout.layout_infoitem, this);
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (index == R.styleable.MyTextItemView_exist_icon) {
                ...
                int color = typedArray.getColor(index, getResources().getColor(R.color.text_noraml_color));
                mTvMyLeftText.setTextColor(color);
            } else if (index == R.styleable.MyTextItemView_right_text_color) {
                int color = typedArray.getColor(index, getResources().getColor(R.color.border_color));
